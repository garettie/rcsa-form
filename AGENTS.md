...
- Read DESIGN.md and PRODUCT.md when doing UI work — they define the project's design system and product personality.
...
- "Add validation" becomes "Verify invalid inputs (empty, malformed, oversized) are rejected, then make them pass."
- "Fix the bug" becomes "Reproduce the reported symptom, identify root cause, fix it, confirm symptom gone."
- "Refactor X" becomes "Existing behavior is preserved before and after, and no public API changes."
- "Make it faster" becomes "Benchmark the current hot path, identify the bottleneck with profiling, change it, show the benchmark is faster."

For every task:

1. State the success criteria before writing code.
2. Write the verification (manual test steps, screenshot, benchmark) where practical.
...
- Prefer lean-ctx MCP tools for token efficiency (see LEAN-CTX.md).
- Prefer running the code to guessing about the code. If a linter exists, run it. If a type checker exists, run it.
- Never report "done" based on a plausible-looking diff alone. Plausibility is not correctness.
- When debugging, address root causes, not symptoms. Suppressing the error is not fixing the error.
- For UI changes, verify visually: screenshot before, screenshot after, describe the diff.
- Use CLI tools (gh, aws, vercel) when they exist. They are more context-efficient than reading docs.
...
- Frameworks: React ^19.2.5, Vite ^8.0.9, TailwindCSS ^4.2.4
- Libraries: Lucide React, Supabase (PostgreSQL)
- Package manager: npm
...
- `src/types.ts`, `src/constants.ts`, `src/mockData.ts`
- Supabase: Auth and data persistence. Client in `src/api.ts`.

### Design system
- `DESIGN.md` at root: full color palette, typography, component specs, layout rules
- `PRODUCT.md` at root: product purpose, users, brand personality, anti-references
- TailwindCSS v4 with `@import "tailwindcss"` (no `tailwind.config.js`)

### Conventions
- Components: PascalCase. One component per file preferred.
- Lean-ctx MCP tools preferred for token savings (see LEAN-CTX.md).
- Conventional commits: `feat:`, `fix:`, `refactor:`, `chore:`, `style:` prefixes.
...
If `tsc -b` or `npm run lint` would catch it, it doesn't go here. If reading the repo would make it obvious, it doesn't go here. One entry per session max; zero is fine.
